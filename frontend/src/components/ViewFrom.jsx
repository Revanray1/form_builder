...
function ViewFrom() {
    ...
    const handleSubmit = (e) => {
        ...
        console.log("Subitted Form data",updatedData)
        // Validation if needs  
        navigate("/")
    };
    useEffect(() => {
                    ...
                    {currentData && currentData?.fields.map((data, index) => {
                        return (<>
                            <div key={index} class="justify-content-center d-flex" style={{ height: "50%", padding: "2%", width: "45%" }}>
                                <span>{data.name} : </span>
                                <input class="input-field-border" type={`${data.type}`} id={`${data.name}`} name={`${data.name}`} defaultValue={`${data.value}`} placeholder={`${data.placeHolder}`} maxLength={20} onChange={handleChange} />
                            </div>
                        </>)
                    }
                    )}
